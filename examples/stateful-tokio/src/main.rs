...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    app.run("0.0.0.0:80").await?;

    Ok(())
}

/// Request handler for the `/` path.
async fn index(_: Request, state: Arc<AppState>) -> Response {
...
async fn add_press(_: Request, state: Arc<AppState>) -> Response {
...
async fn get_presses(_: Request, state: Arc<AppState>) -> Response {
    // Get the number of button presses using the thread-safe `AtomicUsize`
    let presses = state.button_presses.load(Ordering::SeqCst);
