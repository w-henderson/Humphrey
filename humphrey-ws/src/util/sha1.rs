...
pub fn hash<T>(input: T) -> [u8; 20]
where
    T: AsRef<[u8]>,
{
    // Calculate padded message length then perform padding
    let message_len = ((input.as_ref().len() * 8 + 583) / 512) * 64;
    let mut message: Vec<u8> = vec![0; message_len];
    message[0..input.as_ref().len()].copy_from_slice(input.as_ref());
    message[input.as_ref().len()] = 0x80;
    message[message_len - 8..].copy_from_slice(&(input.as_ref().len() * 8).to_be_bytes());
